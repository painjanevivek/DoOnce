...
async function runVerifiedDemoDownload() {
  ...
  if (!(button instanceof HTMLButtonElement) || button.disabled || !status) {
    return { outcome: "paused", reasonCode: "changed-page", reason: "The expected safe download control is unavailable." };
  }
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  void runVerifiedDemoDownload().then(sendResponse, () => sendResponse({ outcome: "paused", reasonCode: "unknown", reason: "The demo run could not be verified." }));
  return true;
});
